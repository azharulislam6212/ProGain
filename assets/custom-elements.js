...
class ResponsiveImage extends HTMLImageElement {
    ...
    get isReady() {
        return this.classList.contains("is-loaded");
    }
    connectedCallback() {
        this.#init();
    }
    ...
    updatedCallback() {
        ...
        currentSrc !== this.#lastSrc &&
            ((this.#lastSrc = currentSrc),
                ...
                this.wrapper?.classList.remove("is-loaded", "is-loading", "error"),
                this.#init());
    }
    #init() {
        ...
        const isLiquid = this.dataset.mode === "liquid",
            ...
            wasAlreadyLoaded = this.classList.contains("is-loaded"),
            isComplete = this.complete && this.naturalWidth > 0;
        if (
            (this.classList.remove("is-loading", "is-loaded"),
                this.wrapper?.classList.remove("is-loading", "is-loaded", "error"),
    ...
    #waitForLoad(usePoll = !1) {
        this.classList.remove("is-loaded"),
            this.wrapper?.classList.remove("is-loaded", "error"),
    ...
    #loadImage() {
        if (!this.classList.contains("is-loaded")) {
    ...
    #markLoaded() {
        this.classList.contains("is-loaded") ||
            (this.classList.remove("is-loading"),
                this.classList.add("is-loaded"),
                this.wrapper?.classList.remove("is-loading"),
                this.wrapper?.classList.add("is-loaded"),
                this.#resolveReady?.(),
